...
const GWPicksTable = () => {
  ...
  return (
    <div className="w-full lg:w-2/3 bg-white p-4 overflow-x-scroll rounded-lg border grid grid-cols-1 gap-4">
      ...
      <div className="w-full pitch py-4 rounded-lg">
        <div className="w-full flex items-center justify-evenly py-4 gap-2 px-2 cursor-pointer">
          {startingXI
    ...
    >
      ...
      <div className="absolute flex flex-col right-0 top-0 gap-2">
        {(l.is_captain || l.is_vice_captain) && (
          <p className="text-blue bg-white w-6 h-6 text-sm flex items-center justify-center rounded-md">
            {l.is_captain && "(C)"} {l.is_vice_captain && "(V)"}
          </p>
        )}

        {(l.subbed_in || l.subbed_out) && (
          <p className="bg-white w-6 h-6 text-sm flex items-center justify-center rounded-md">
